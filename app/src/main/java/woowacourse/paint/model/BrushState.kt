...
interface BrushState {
    fun draw(canvas: Canvas, currentPath: Path?, currentPaint: Paint)
    fun start(x: Float, y: Float): Path
    fun move(path: Path, x: Float, y: Float)
    fun finish(path: Path, paint: Paint)
}

class PenState(private val strokes: Strokes) : BrushState {
    override fun draw(canvas: Canvas, currentPath: Path?, currentPaint: Paint) {
        currentPath?.let { canvas.drawPath(it, currentPaint) }
    }

    override fun start(x: Float, y: Float): Path = Path().apply { moveTo(x, y) }

    override fun move(path: Path, x: Float, y: Float) {
        path.lineTo(x, y)
    }

    override fun finish(path: Path, paint: Paint) {
...
class RectangularState(private val strokes: Strokes) : BrushState {
    ...
    override fun start(x: Float, y: Float): Path {
        startX = x
        startY = y
        return Path()
    }
