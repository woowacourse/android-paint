...
class Drawn {
    ...
    fun startDraw(x: Float, y: Float, paint: Paint) {
        setDraw(x, y, paint)
    ...
    fun changeBrushType(brushType: BrushType, paint: Paint) {
        brush = brush.changeType(brushType)
        setDraw(paint = paint)
    }

    fun changeLineWidth(width: Float, paint: Paint) {
        brush = brush.changeWidth(width)
        setDraw(paint = paint)
    }

    fun changeColor(colorPalette: ColorPalette, paint: Paint) {
        brush = brush.changeColor(colorPalette)
        setDraw(paint = paint)
    }

    private fun setDraw(x: Float = 0f, y: Float = 0f, paint: Paint) {
        currentDraw = when (brush.brushType) {
            BrushType.PEN -> {
